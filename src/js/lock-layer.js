...
async function run() {
  ...
  try {
    ...
    // Test functions here
    engine.block.setScopeEnabled(graphic, 'layer/resize', false);
    const canResize = engine.block.isScopeEnabled(graphic, 'layer/resize');
    console.log('layer/resize scope enabled?', canResize);

    // Download result
    const blob = await engine.block.export(graphic, { mimeType: 'image/png' });
    const outputFilename = `lock-layer-${Date.now()}.png`;
